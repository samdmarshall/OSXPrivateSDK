//
//     Generated by class-dump 3.5 (64 bit).
//
//     class-dump is Copyright (C) 1997-1998, 2000-2001, 2004-2013 by Steve Nygard.
//

#import <CalendarPersistence/IIQuerySession.h>

@class CalDAVAccount, CalDAVPrincipalSearchPropertySet, NSArray, NSMutableArray, NSMutableDictionary, NSString;

@interface IICDQuerySession : IIQuerySession
{
    CalDAVAccount *_account;
    CalDAVPrincipalSearchPropertySet *_searchSet;
    NSString *_searchString;
    NSString *_collectionSetName;
    NSArray *_searchTypes;
    NSMutableArray *_newResults;
    NSMutableArray *_oldResults;
    NSMutableDictionary *_combinedResults;
    BOOL _isRunning;
}

@property(retain) NSString *collectionSetName; // @synthesize collectionSetName=_collectionSetName;
@property(retain) NSArray *searchTypes; // @synthesize searchTypes=_searchTypes;
@property(retain) NSString *searchString; // @synthesize searchString=_searchString;
@property(retain) CalDAVPrincipalSearchPropertySet *searchSet; // @synthesize searchSet=_searchSet;
@property(retain) CalDAVAccount *account; // @synthesize account=_account;
- (id)results;
- (void)_handleOperationResults:(id)arg1;
- (void)_launchAgentSearch:(id)arg1;
- (id)searchFor:(id)arg1 ofTypes:(id)arg2 withAccount:(id)arg3;
- (void)loadPreviousSearch:(id)arg1 forKey:(id)arg2;
- (void)resetQuery;
- (BOOL)isRunning;
- (void)dealloc;
- (id)initWithDelegate:(id)arg1;

@end

